...
export const tokenizePDF = async (docId: string | URL | TypedArray | ArrayBuffer | DocumentInitParameters) => {
	const doc = await pdfjsLib.getDocument(docId).promise;

	return async <R>(pageNum: number, f: (d: Token[]) => Promise<R>): Promise<R> => {
